 void createLocalA(Matrix &A,mesh m){
    float thao = m.getParameter(THAO);
    A.at(0).at(0) += -thao/8;  A.at(0).at(1) += thao/8;
    A.at(1).at(0) += -thao/8;  A.at(1).at(1) += thao/8;
}

void createLocalB(Matrix &B,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float kap = m.getParameter(KAPPA);
    B.at(0).at(0) += kap/l;      B.at(0).at(1) += -kap/l;
    B.at(1).at(0) += -kap/l;     B.at(1).at(1) += kap/l;
}

void createLocalC(Matrix &C,mesh m){
    float delta = m.getParameter(DELTA);
    C.at(0).at(0) += (-1*delta)/3;    C.at(0).at(1) += (delta)/3;
    C.at(1).at(0) += (-1*delta)/3;    C.at(1).at(1) += (delta)/3;
}

void createLocalD(Matrix &D,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float v = m.getParameter(VELOCITY);
    D.at(0).at(0) += v/l;  D.at(0).at(1) += (-1*v)/l;
    D.at(1).at(0) += (-1*v)/l;  D.at(1).at(1) += v/l;
}

 void createLocalE(Matrix &D,mesh m){
     float alpha = m.getParameter(ALPHA);
     D.at(0).at(0) += (-3*alpha)/2;  D.at(0).at(1) += (3*alpha)/2;
     D.at(1).at(0) += (-3*alpha)/2;  D.at(1).at(1) += (3*alpha)/2;
 }


 void createLocalH(Matrix &D,mesh m){
     float delta = m.getParameter(DELTA);
     D.at(0).at(0) += (-delta)/2;  D.at(0).at(1) += (delta)/2;
     D.at(1).at(0) += (-delta)/2;  D.at(1).at(1) += (delta)/2;
 }
...
Vector createLocalb(int element,mesh &m){
    ...
    float psi = m.getParameter(PSI), l = m.getParameter(ELEMENT_LENGTH);
